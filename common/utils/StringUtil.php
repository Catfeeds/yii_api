...
class StringUtil
{
    ...
    // 随机字符串
    public static function createStr($length = 16)
    {
     ...
     *            要多长
     * @param integer $numberic
     *            数字还是字符串
     * @return string $hash 返回字符串
     */
    public static function random($length, $numeric = 1)
    {
    ...
    public static function getUidAByUDPX($udpX, $findC = false, $returnDisable = false, $returnRelated = true)
    {
        ...
        if ($findC) {
            $diff = array_intersect($udpA, array(
                'c_0',
                'alldept'
            ));
            if (! empty($diff)) {
    ...
    public static function ConvertBytes($sizeMixed)
    {
        ...
        return array_map(function ($temp) {
            return eval("return " . preg_replace('/pb?/i', ' * 1024 * 1024 * 1024 * 1024 * 1024 ', preg_replace('/tb?/i', ' * 1024 * 1024 * 1024 * 1024 ', preg_replace('/gb?/i', ' * 1024 * 1024 * 1024 ', preg_replace('/mb?/i', ' * 1024 * 1024 ', preg_replace('/kb?/i', ' * 1024 ', preg_replace('/b?/i', '', $temp)))))) . ";");
        }, $sizeArray);
    }
