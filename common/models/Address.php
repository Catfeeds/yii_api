...
class Address extends \common\components\ETActiveRecord
{
    ...
    {
        return [
            [['user_id', 'created_at', 'created_by', 'is_default', 'status', 'updated_at', 'updated_by'], 'integer'],
    ...
    {
        return $this->hasOne(Area::className(), ['id' => 'address_id']);
    }
    ...
    public static function findByUser($user_id)
    {
        return static::find()->where('user_id=:user_id ',
            ['user_id'=>$user_id ])
            ->with(['area'])->orderBy('is_default desc')->asArray()->all();
    }
}
