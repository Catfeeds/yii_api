...
use common\extensions\Wechat\lib\Tools;
...
class WechatOauth extends Common {
    ...
    const OAUTH_JSCODE2SESSION = '/sns/jscode2session?';
    /**
     * Oauth 授权跳转接口
    ...
    public function getOauthOpenid() {
        $code = isset($_GET['js_code']) ? $_GET['js_code'] : '';
        if (empty($code)) {
            Tools::log("getOauthAccessToken Fail, Because there is no access to the code value in get.");
            return false;
        }
        $result = Tools::httpGet(self::API_BASE_URL_PREFIX . self::OAUTH_JSCODE2SESSION . "appid={$this->appid}&secret={$this->appsecret}&js_code={$code}&grant_type=authorization_code");
        if ($result) {
